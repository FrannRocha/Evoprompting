...
**Muestra la calificación en tu respuesta:** al evaluar una tarea no trivial, abre tu respuesta en el chat con la línea de calificación — el usuario debe verla desde Claude, siempre, sin tener que ir a GitHub:

**📊 Dificultad: 6/10 · Prompt: 7/10**

Solo los números, en negritas, SIN barras de bloques (`█`/`░`) — esos caracteres se renderean como barras blancas en varias interfaces y ensucian la respuesta. **Dificultad** = esta escala. **Prompt** = qué tan bien planteada estaba la petición con la que se ejecutó (claridad, contexto, restricciones; sube si el usuario la dejó evopromptear en §7). Después de la línea, las razones de cada nota **y, si Prompt ≤ 6, UNA línea con cómo plantear la próxima iteración mejor** — la nota existe para iterar, no para regañar. Si el trabajo además termina en un pull request, repite la misma línea al final del título y al inicio del cuerpo — GitHub no permite insertar datos propios junto a sus contadores `+/−` del diff, y el título del PR aparece en esa misma fila de la lista.

**Alimenta la barra integrada (statusline):** cada vez que califiques, sobrescribe además el archivo `~/.claude/evo-score` con exactamente dos líneas: `DF=N` y `PROMPT=M` (créalo si no existe). El `statusline.sh` de este repo lee ese archivo y muestra `DF N/10 · Promt M/10` con colores en la barra nativa de Claude Code — integrado a la UI, como los contadores de GitHub. Se instala con `install.sh` (ver README); con `install.sh --siempre` la calificación corre en todos los proyectos del usuario, aunque no usen Evoprompting.
<!-- EVO · DIFICULTAD: escala 1-10 con criterios (documentación, claridad, probabilidad), umbrales de acción, barra de calificación visible (Dificultad + Prompt) y alimentación de la statusline vía ~/.claude/evo-score. POR QUÉ: pedida por Francisco — si por probabilidad es muy improbable resolverlo (ej. sin documentación), el modelo debe decirlo y mejorar la petición antes de ejecutar, no descubrirlo tras 3 días de iteración; los umbrales convierten el número en decisión, no en decoración; la barra existe porque Francisco quería ver la calificación como ve los contadores +/− de GitHub, y el archivo evo-score es el puente hacia la UI nativa (statusline). FUENTE: Francisco 2026-07-02. EFECTIVIDAD: falló la barra de bloques █░ (2026-07-02: en la UI se veía como "barras blancas feas" — feedback de Francisco); la línea numérica limpia está en prueba. CAMBIOS: 2026-07-02 → creada. 2026-07-02(2) → +regla de calificación visible en PRs (`Dificultad: N/10 · Prompt: M/10` en título y cuerpo). 2026-07-02(3) → la barra se muestra SIEMPRE en la respuesta de Claude (pedido de Francisco: "debe verse desde Claude"); el PR conserva la línea en el título y la barra en el cuerpo. 2026-07-02(4) → +barra integrada a la UI: las notas se escriben en ~/.claude/evo-score y statusline.sh las pinta en la barra nativa de Claude Code; +tip de mejora cuando Prompt ≤ 6 (pedido de Francisco: "totalmente integrado como natural de Claude" y "que ayude a iterar"). 2026-07-02(5) → ELIMINADAS las barras de bloques █░ del formato de chat y de PR (reconstrucción §8 sobre feedback de Francisco: se veían como barras blancas feas); queda solo la línea `📊 Dificultad: N/10 · Prompt: M/10` en negritas. -->
