...
Un `█` por punto y `░` hasta completar 10, dentro de un bloque de código para que se alinee. **Dificultad** = esta escala. **Prompt** = qué tan bien planteada estaba la petición con la que se ejecutó (claridad, contexto, restricciones; sube si el usuario la dejó evopromptear en §7). Después de la barra, una línea con las razones de cada nota. Si el trabajo además termina en un pull request, repite la calificación al final del título (`· Dificultad: N/10 · Prompt: M/10`) y la barra al inicio del cuerpo — GitHub no permite insertar datos propios junto a sus contadores `+/−` del diff, y el título del PR aparece en esa misma fila de la lista.
<!-- EVO · DIFICULTAD: escala 1-10 con criterios (documentación, claridad, probabilidad), umbrales de acción y barra de calificación visible (Dificultad + Prompt). POR QUÉ: pedida por Francisco — si por probabilidad es muy improbable resolverlo (ej. sin documentación), el modelo debe decirlo y mejorar la petición antes de ejecutar, no descubrirlo tras 3 días de iteración; los umbrales convierten el número en decisión, no en decoración; la barra existe porque Francisco quería ver la calificación como ve los contadores +/− de GitHub. FUENTE: Francisco 2026-07-02. EFECTIVIDAD: sin medir. CAMBIOS: 2026-07-02 → creada. 2026-07-02(2) → +regla de calificación visible en PRs (`Dificultad: N/10 · Prompt: M/10` en título y cuerpo). 2026-07-02(3) → la barra se muestra SIEMPRE en la respuesta de Claude (pedido de Francisco: "debe verse desde Claude"); el PR conserva la línea en el título y la barra en el cuerpo. -->
